...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Employee\\Google Drive\\UP\\MyResearch\\TaskProgramming\\AB_pic\\AB_pic_lastrun.py',
...
option2_t1 = visual.ImageStim(
    ...
    name='option2_t1', 
    image='sin', mask=None,
    ori=0, pos=(0, .30), size=(0.25, 0.25),
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
    texRes=128, interpolate=True, depth=-4.0)
option3_t1 = visual.ImageStim(
    win=win,
    name='option3_t1', 
    image='sin', mask=None,
    ori=0, pos=(.3, 0), size=(0.25, 0.25),
...
for thisTrial in trials:
    ...
    trialComponents = [polygon, polygon_3, displayPicture, polygon_2]
    ...
    while continueRoutine:
        ...
        if polygon_3.status == STARTED:  # only update if drawing
            polygon_3.setOpacity(curOpacity3)
    ...
    option4_t1.setImage(opt4)
    option1_t1.setImage(opt1)
    option2_t1.setImage(opt2)
    option3_t1.setImage(opt3)
    key_resp.keys = []
    key_resp.rt = []
    _key_resp_allKeys = []
    # keep track of which components have finished
    response1Components = [polygon_4, option4_t1, option1_t1, option2_t1, option3_t1, key_resp]
    ...
    while continueRoutine:
        ...
        # *option4_t1* updates
        if option4_t1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            option4_t1.frameNStart = frameN  # exact frame index
            option4_t1.tStart = t  # local t and not account for scr refresh
            option4_t1.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(option4_t1, 'tStartRefresh')  # time at next scr refresh
            option4_t1.setAutoDraw(True)
        ...
        # *option3_t1* updates
        if option3_t1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            option3_t1.frameNStart = frameN  # exact frame index
            option3_t1.tStart = t  # local t and not account for scr refresh
            option3_t1.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(option3_t1, 'tStartRefresh')  # time at next scr refresh
            option3_t1.setAutoDraw(True)
    ...
    trials.addData('option4_t1.started', option4_t1.tStartRefresh)
    trials.addData('option4_t1.stopped', option4_t1.tStopRefresh)
    ...
    trials.addData('option3_t1.started', option3_t1.tStartRefresh)
    trials.addData('option3_t1.stopped', option3_t1.tStopRefresh)
    # check responses
    ...
    option3_t2.setImage(opt3)
    option4_t2.setImage(opt4)
    key_resp_2.keys = []
    key_resp_2.rt = []
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    response2Components = [polygon_5, option1_t2, option2_t2, option3_t2, option4_t2, key_resp_2]
    ...
    while continueRoutine:
        ...
        # *option3_t2* updates
        if option3_t2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            option3_t2.frameNStart = frameN  # exact frame index
            option3_t2.tStart = t  # local t and not account for scr refresh
            option3_t2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(option3_t2, 'tStartRefresh')  # time at next scr refresh
            option3_t2.setAutoDraw(True)
        
        # *option4_t2* updates
        if option4_t2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            option4_t2.frameNStart = frameN  # exact frame index
            option4_t2.tStart = t  # local t and not account for scr refresh
            option4_t2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(option4_t2, 'tStartRefresh')  # time at next scr refresh
            option4_t2.setAutoDraw(True)
    ...
    trials.addData('option3_t2.started', option3_t2.tStartRefresh)
    trials.addData('option3_t2.stopped', option3_t2.tStopRefresh)
    trials.addData('option4_t2.started', option4_t2.tStartRefresh)
    trials.addData('option4_t2.stopped', option4_t2.tStopRefresh)
    # check responses
